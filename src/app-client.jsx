...
// ─────────────────────────────────────────────
//  STORAGE (in-memory with localStorage attempt)
// ─────────────────────────────────────────────
const memStore = {};
const storage = {
...
function LoginScreen({ onBack, onSuccess }) {
  ...
  function tryLogin() {
    const c = getClients().find(x=>x.code===pin.trim());
...
function MemberHomeTab({ client, onTabChange }) {
  ...
  const cycleStart = (() => { try { const r=storage.get("cycleStart"); return r?r.value:""; } catch { return ""; } })();
  const cycleLen = (() => { try { const r=storage.get("cycleLen"); return r?parseInt(r.value):28; } catch { return 28; } })();
  ...
  const cards = [
    { id:"checkin",   icon:"📋", title:"Daily Check-in",              desc:checkedIn?"✅ سجّلتِ اليوم":"سجّلي يومك الآن", color:C.pink,  tab:"home" },
      ...
      {sub && (
        <div style={{ background:"linear-gradient(135deg,#FDF0F4,#F5EBF8)", border:`1px solid ${C.border}`, borderRadius:16, padding:"14px 18px", marginBottom:16 }}>
          ...
          <div style={{ marginTop:8, fontSize:12, color:C.green, fontWeight:700 }}>
            🗓️ سيشنك القادم: {getNextSessionDate(client.sessionDay??0)}
          </div>
        </div>
      )}
...
function CheckInTab({ client }) {
  ...
  function submit() {
    try {
      storage.set(`log:${client.name}:${TODAY}`, JSON.stringify({...log,date:TODAY,client:client.name,submittedAt:new Date().toISOString()}));
      let ex=[]; try{const r=storage.get(`loglist:${client.name}`);if(r)ex=JSON.parse(r.value);}catch{}
      storage.set(`loglist:${client.name}`,JSON.stringify([TODAY,...ex.filter(d=>d!==TODAY)].slice(0,90)));
      setSubmitted(true);
    } catch { alert("خطأ — حاولي تاني"); }
  }
...
function CycleTab({ client }) {
  const [cycleStart, setCycleStart] = useState(()=>{ try{const r=storage.get("cycleStart");return r?r.value:"";}catch{return "";} });
  const [cycleLen, setCycleLen]     = useState(()=>{ try{const r=storage.get("cycleLen");return r?parseInt(r.value):28;}catch{return 28;} });
      ...
      <div style={{ paddingTop:20 }}>
        ...
        {!cycleStart ? (
          <div style={{ ...card, textAlign:"center" }}>
            ...
            <input type="date" value={cycleStart} onChange={e=>{setCycleStart(e.target.value);storage.set("cycleStart",e.target.value);}}
            ...
            <div style={{ display:"flex", gap:10, alignItems:"center" }}>
              ...
              <input type="number" min="21" max="40" value={cycleLen} onChange={e=>{setCycleLen(+e.target.value);storage.set("cycleLen",e.target.value);}}
          ...
          <>
            ...
            <button onClick={()=>{setCycleStart("");storage.del("cycleStart");}} style={{ width:"100%", padding:"10px 0", borderRadius:12, background:C.bg, border:`1px solid ${C.border}`, color:C.muted, fontSize:12, fontWeight:600, cursor:"pointer" }}>
              🔄 تحديث تاريخ الدورة
            </button>
          </>
        )}
...
function TrackerTab({ client }) {
  const [hLog, setHLog] = useState(()=>{
    try{const r=storage.get(`hsym:${client.name}:${TODAY}`);return r?JSON.parse(r.value):{};}catch{return {};}
  });
  const setH = (k,v) => setHLog(p=>{ const n={...p,[k]:v}; storage.set(`hsym:${client.name}:${TODAY}`,JSON.stringify(n)); return n; });

  const cycleStart = (() => { try{const r=storage.get("cycleStart");return r?r.value:"";}catch{return "";} })();
  const ci = getCycleInfo(cycleStart);
  const phase = ci?.phase;
  const phaseColor = phase?.color || C.pink;
  const phaseBg = phase?.bg || C.blush;
    ...
    <div style={wrap}>
      <div style={{ paddingTop:20 }}>
        ...
        <div style={{ ...card, background:"linear-gradient(135deg,#E6F5EE,#F5FBF7)", border:`1px solid ${C.greenBorder}` }}>
          ...
          <div style={{ fontSize:11, color:C.sub, fontWeight:500 }}>الدكتورة ستراها في السيشن القادم</div>
        </div>
      </div>
    </div>
  );
...
function ChatTab({ client }) {
  const [msgs, setMsgs] = useState(()=>{
    try{const r=storage.get(`chat:${client.name}`);return r?JSON.parse(r.value):[];}catch{return [];}
  });
  ...
  function send() {
    ...
    const userMsg = { role:"user", text:input, time:new Date().toLocaleTimeString("ar-EG",{hour:"2-digit",minute:"2-digit"}) };
    const newMsgs = [...msgs, userMsg];
    setMsgs(newMsgs); setInput(""); setSending(true);
    const key = `clientmsg:${client.name}:${TODAY}:${Date.now()}`;
    storage.set(key, JSON.stringify({text:input,from:client.name,date:TODAY,sentAt:new Date().toISOString()}));
    setTimeout(()=>{
      const reply = { role:"doctor", text:"✅ وصلتني رسالتك يا حبيبتي! سأرد عليكِ في أقرب وقت 🌸", time:new Date().toLocaleTimeString("ar-EG",{hour:"2-digit",minute:"2-digit"}) };
...
export default function App() {
  ...
  useEffect(()=>{
    try {
      ...
      if (saved?.value) {
        const c = getClients().find(x=>x.name===saved.value);
